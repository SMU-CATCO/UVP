...
The GUI requires three input files: an RP curvature file (.dat format), an RC energy file (.csv format), and an RC geometry file (.xyz format). These output files are generated by our group's URVA2025 program and its associated post-processing scripts,<sup>1</sup> which will be provided upon request (contact email: ekraka@mail.smu.edu). Examples of these input files are provided in the examples directory and are also discussed below.
